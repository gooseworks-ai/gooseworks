 ...
 * concept batches, anything that is not a recipe order) still render on the
 * user's own Claude Code: the app pre-creates the project and hands the user a
 * paste prompt; this skill fetches the per-format recipe, renders on the user's
 * machine (Playwright + ffmpeg + the GooseWorks media proxies), mirrors the
 * script for a free in-app review, then saves the finished MP4 back over MCP.
 * It cannot be retired until a server worker consumes those renders
 * (`submit_render` alias: sunset "after-render-worker").
 */
